...
/*************************************************
* Name:        crypto_sign_signature_faulty
*
* Description: Computes signature.
*
* Arguments:   - uint8_t *sig: pointer to output signature (of length CRYPTO_BYTES)
...
* Returns 0 (success)
**************************************************/
int crypto_sign_signature_faulty(uint8_t *sig, size_t *siglen, const uint8_t *m, size_t mlen, const uint8_t *sk, const size_t row, const size_t cff) {
  unsigned int i, n, pos;
  uint8_t seedbuf[3*SEEDBYTES + 2*CRHBYTES];
  uint8_t *rho, *tr, *key, *mu, *rhoprime;
  uint8_t hintbuf[N];
  ...
  randombytes(rhoprime, CRHBYTES);
#else
#warning "not randomized"
  shake256(rhoprime, CRHBYTES, key, SEEDBYTES + CRHBYTES);
#endif

  /* Expand matrix and transform vectors */
  polyvec_matrix_expand(mat, rho);
  mat[0].vec[row].coeffs[unpack_map[cff]] = 0; // FAULT
  polyvecl_ntt(&s1);
  polyveck_ntt(&s2);
  polyveck_ntt(&t0);
